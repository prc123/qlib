...
def get_proxies(authKey,password):
#青果网络代理ip
        response = requests.get(f"https://overseas.proxy.qg.net/get?key={authKey}")
...
class YahooCollector(BaseCollector):
    ...
    def get_data_from_remote(symbol, interval, start, end, show_1min_logging: bool = False):
        ...
        try:
            _resp = Ticker(symbol,proxies = PROXIES_1,asynchronous=False).history(interval=interval, start=start, end=end)
