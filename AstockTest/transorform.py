...
# 辅助函数：创建数据加载器
def create_data_loader_from_dataset(dataset: StockDataset_min_v2, batch_size: int = 32, shuffle: bool = True) -> DataLoader:
    """
    从StockDataset_min_v2创建数据加载器
    
    Args:
        dataset: StockDataset_min_v2数据集实例
        batch_size: 批次大小
        shuffle: 是否打乱数据
...
if __name__ == "__main__":
    ...
    model = SimpleTransformerModel(input_size=8, d_model=128, nhead=8, num_layers=4, seq_len=10)
    print(model)
    ...
    try:
        # 示例：加载股票数据
        file_name = r'C:\Users\Administrator\Documents\stock_ananisy\out_process_stock\df_out_process_stock_20250929_min_processed.csv'
        stock_data = pd.read_csv(file_name)
        stock_data = stock_data.dropna()
        #stock_data  = stock_data[:5000]
        print(stock_data.head())
        train_df,test_df,vaild_df = get_train_test_vaild(stock_data)
